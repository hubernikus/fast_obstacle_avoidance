...
from fast_obstacle_avoidance.obstacle_avoider import (
    SampledAvoider, FastObstacleAvoider, MixedEnvironmentAvoider
    )
...
def plot_environment(ax, my_avoider,
                     positions=None, velocities_mod=None,
                     dynamical_system=None,
                     x_lim=None, y_lim=None,
                     draw_velocity_arrow=False):
    if hasattr(my_avoider, 'obstacle_avoider'):
        plot_obstacles(
            ax,
...
def visualization_sampledata(ax, qolo, dynamical_system, x_lim, y_lim, n_sampels=10):
    ...
    #     color='red',
    # )
    plot_environment(ax, my_avoider, positions, velocities_mod, dynamical_system, x_lim, y_lim)


def visualization_obstacle(qolo, dynamical_system):
    pass

    
def visualization_mixed(ax, qolo, dynamical_system, x_lim, y_lim, n_sampels=10):
    """Draw the vectorfield mixed"""
    qolo._got_new_obstacles = True
    ...
    # mixed_avoider.update_laserscan(qolo.get_allscan())
    # mixed_avoider.update_reference_direction()
    ...
    plot_environment(ax, mixed_avoider, positions, velocities_mod, dynamical_system, x_lim, y_lim)
...
def time_integration_fast(
    avoider, dynamical_system, it_max=350, dt=0.1, conv_margin=1e-1, conv_margin_ds=1e-2,
    ax=None, lasercan=None):
    ...
    while it < it_max:
        ...
        if LA.norm(avoider.robot.pose.position-dynamical_system.attractor_position) < conv_margin:
            final_it = it
            break
        ...
        if (hasattr(avoider, 'obstacle_environment') and
            is_interesecting(avoider.obstacle_environment, positions[:, it])):
            # Stuck / colliding 
            final_it = -3
            break
...
def compare_numerically_algorithms(qolo):
    ...
    plot_environment(
        ax=axs[0], my_avoider=mixed_avoider,
        dynamical_system=dynamical_system,
        x_lim=x_lim, y_lim=y_lim,
        )

    plot_environment(
        ax=axs[1], my_avoider=sample_avoider,
        dynamical_system=dynamical_system,
        x_lim=x_lim, y_lim=y_lim,
        )
...
def compare_two_vectorfields(qolo, save_figure=False, n_sampels=100):
    ...
    visualization_mixed(ax, qolo, dynamical_system, x_lim, y_lim, n_sampels=n_sampels)

    if save_figure:
        figure_name = "mixed_environment_obstacle"
...
if (__name__) == "__main__":
    ...
    # compare_two_vectorfields(qolo=qolo, save_figure=False, n_sampels=10)
