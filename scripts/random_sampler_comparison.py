...
def single_sample_run(avoider, dimensions, num_samples, initial_velocity, stretching_factor=1000):
    it_repeat = 0
    ...
    if np.sum(ind_close):
        try:
            data[:, ind_close] = data[:, ind_close] + np.ones((dimensions, np.sum(ind_close)))
        except:
            breakpoint()

    t_start = timer()
    avoider.update_reference_direction(
        data, in_robot_frame=False)
...
def create_obstacle_container(dim, n_obstacles, ax_range=np.array([0.5, 2]), pos_scaling=10):
    ...
    for ii in range(n_obstacles):
        ...
        my_container.append(
             Ellipse(center_position=position,
                     axes_length=axes)
             )
