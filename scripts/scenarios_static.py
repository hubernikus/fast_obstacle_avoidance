...
def explore_specific_point():
    qolo = QoloRobot(
        pose=ObjectPose(position=[7.0, -2.0], orientation=0)
    )
    ...
    main_environment = ShapelySamplingContainer(n_samples=20)
    ...
    ax.arrow(eval_pos[0], eval_pos[1],
...
def normal_offset_around_edge():
    ...
    qolo = QoloRobot(
        pose=ObjectPose(position=eval_pos, orientation=0)
    )
